    ...
    <div id="results">
      ...
      <v-form @submit.prevent="savePost" v-model="valid" ref="itemData">
        <v-container style="max-width=200px">
          <v-row class="ml-0">
            <p>
              Fill in the optional fields below to see filtered search results.
            </p>
          </v-row>
          <v-row>
            <v-col cols="12" sm="6">
      ...
      <v-row>
        <v-col sm="12">
          ...
          <p class="mb-3 ml-2 pb-10" v-if="items.length == 0 && submitStart">
            There are no results for your query. Check whether you misspelled
            something, and make sure that you really do want all of the filters
            you've set.
          </p>
  ...
  methods: {
    createPDF() {
      ...
      doc.text(
        ...
        this.currentuser.family_name +
          " | Report generated on app.stockpileapp.au"
      );
      ...
      if (this.sBuyer != "") {
        doc.text(35, txtLocation, "Buyer: " + this.sBuyer);
        txtLocation += 5;
      }

      if (this.sCat != "") {
        doc.text(35, txtLocation, "Category: " + this.sCat);
        txtLocation += 5;
      }

      if (this.sPrice != "") {
        doc.text(35, txtLocation, "Price: $" + this.sPrice);
        txtLocation += 5;
      }

      let sDateMod = this.sDate.map((x) =>
        new Date(x).toLocaleDateString("en-AU")
      );
      ...
      if (sDateMod[0] != null) {
        doc.text(
          35,
          txtLocation,
          "Date range: " + sDateMod[0] + " ~ " + sDateMod[1]
        );
        txtLocation += 5;
      }
      ...
      doc.autoTable(columnHeader, rows, { startY: txtLocation });
    ...
    submitAdvancedSearch() {
      ...
      for (var i = 0; i < this.itemsBackup.length; i++) {
        ...
        var cardDate = new Date(ib[i].datePurchased).toLocaleDateString(
          "en-AU"
        );
        var sDateMod = this.sDate.map((x) =>
          new Date(x).toLocaleDateString("en-AU")
        );
        ...
        if (
          ...
          (sDateMod[0] == null ||
            ...
            this.isDateBetween(sDateMod[1], sDateMod[0], cardDate)) &&
